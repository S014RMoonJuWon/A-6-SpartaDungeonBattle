...
public class GameManager
{
    ...
    // 3차 : 유창님 자료 진행 후 확인 필요사항 주석남김
    {
        ...
        for (int i = 0; i < enemyCount; i++) // 이전에 enemy.count였음_주원님이 수정 작성함
        {
            ...
            //확인필요! (주원님 자료에는 없음/재원님, 유창님 자료에는 있음)
            Console.WriteLine($"{i + 1} : {randomEnemy.Name} Hp {randomEnemy.Hp}");
        }

        foreach (Enemy randomEnemy in randomEnemies) //주원님 설정
        {
        ...
        {
            case 1:
                BattleMenu(enemyCount, randomEnemies); // 주원님 설정
                break;
        }
        // Attck, Skill 함수는 enemyCount 수 까지 누를 수 있게

    }
    private void BattleMenu(int enemyCount, List<Enemy> randomEnemies)
    {
        // 여기부터(주원님 자료)
        int index = 1;
        ...
        // !!여기부터 다시 봐야 함!!
        // 최초 여기부터 BattleMenu의 시작이었음
        // 주원님, 유창님 자료에는 여기부터 없음
        // 재원님 자료에는 여기부터 있음
    ...
    private int SelectEnemy(List<Enemy> enemies) // 재원님이 작성한 내용
    {
